...
static int blb_sqlite_query( thread_t* th,const query_t* q );
static int blb_sqlite_input( thread_t* th,const input_t* i );
static void blb_sqlite_dump( thread_t* th,const dump_t* d );
static void blb_sqlite_backup( thread_t* th,const backup_t* b );

static const dbi_t blb_sqlite_dbi={
    .thread_init=blb_sqlite_thread_init
...
static int blb_sqlite_input( thread_t* th,const input_t* i ){
    ...
    sqlite3_bind_text(db->insert_stmt,SQLITE_BIND_RRNAME_IDX,i->rrname,i->rrname_len,SQLITE_STATIC);
    sqlite3_bind_text(db->insert_stmt,SQLITE_BIND_RRTYPE_IDX,i->rrtype,i->rrtype_len,SQLITE_STATIC);
    sqlite3_bind_text(db->insert_stmt,SQLITE_BIND_RDATA_IDX,i->rdata,i->rdata_len,SQLITE_STATIC);
    sqlite3_bind_text(db->insert_stmt,SQLITE_BIND_SENSORID_IDX,i->sensorid,i->sensorid_len,SQLITE_STATIC);
    sqlite3_bind_int(db->insert_stmt,SQLITE_BIND_COUNT_IDX,i->count);
    sqlite3_bind_int(db->insert_stmt,SQLITE_BIND_FIRSTSEEN_IDX,i->first_seen);
    sqlite3_bind_int(db->insert_stmt,SQLITE_BIND_LASTSEEN_IDX,i->last_seen);

    int step_ok=sqlite3_step(db->insert_stmt);
    if( step_ok!=SQLITE_DONE ){
        L(prnl("sqlite3_step() failed with rc `%d`",step_ok));
...
db_t* blb_sqlite_open( const blb_sqlite_config_t* config ){
    ...
    V(prnl("sqlite database is `%s`",config->path));

    if( strcmp(config->journal_mode,"wal")!=0
      &&strcmp(config->journal_mode,"memory")!=0 ){
        L(prnl("unknown journal_mode `%s`",config->journal_mode));
        blb_free(db);
        return(NULL);
    }
    ...
    if( rc!=SQLITE_OK ){
        L(prnl("sqlite_open() failed with `%s`",sqlite3_errmsg(db->db)));
        ASSERT( db->db!=NULL );
        sqlite3_close(db->db);
        blb_free(db);
        return(NULL);
    ...
    (void)snprintf(_create_table_stmt,sizeof(_create_table_stmt),_create_table,config->journal_mode);

    char* err=NULL;
    int stmt_ok=sqlite3_exec(db->db,_create_table_stmt,NULL,NULL,&err);
    if( stmt_ok!=SQLITE_OK ){
        ASSERT( err!=NULL );
        L(prnl("sqlite3_exec() failed with `%s`",err));
        sqlite3_free(err);
        sqlite3_close(db->db);
        blb_free(db);
        return(NULL);
    ...
    int insert_stmt_ok=sqlite3_prepare_v2(db->db,_insert_stmt,strlen(_insert_stmt),&db->insert_stmt,NULL);
    if( insert_stmt_ok!=SQLITE_OK ){
        L(prnl("sqlite3_prepare_v2() failed with `%s`",sqlite3_errmsg(db->db)));
        sqlite3_close(db->db);
        blb_free(db);
        return(NULL);
    }
