...
static inline uint32_t bs_read_w16_le( bytestring_source_t* bs,int* ok) {
    if( bs->index+2>bs->available ){if( ok!=NULL ){*ok=-1;}return(0);}
...
static inline uint32_t bs_read_w32_le( bytestring_source_t* bs,int* ok) {
    if( bs->index+4>bs->available ){if( ok!=NULL ){*ok=-1;}return(0);}
...
static inline uint64_t bs_read_w64_le( bytestring_source_t* bs,int* ok) {
    if( bs->index+8>bs->available ){if( ok!=NULL ){*ok=-1;}return(0);}
...
static inline int bs_append( bytestring_sink_t* bs,const uint8_t* p,size_t n ){
...
static inline bytestring_sink_t bs_sink_slice0( const bytestring_sink_t* bs,size_t n ){
    ...
    return((bytestring_sink_t){.p=bs->p+bs->index+n,.index=0,.available=bs->available-n});
}

#define bs_source(data,n)(bytestring_source_t){.p=(data),.index=0,.available=(n)}
#define bs_sink(data,n)(bytestring_sink_t){.p=(data),.index=0,.available=(n)}
...
static inline int bs_append_escape( bytestring_sink_t* bs,const uint8_t* p,size_t n ){
    int64_t ok=0;
    for( size_t i=0;i<n;i++ ){
        ok+=bs_sink_escape(bs,p[i]);
    }
    return(ok<0?-1:0);
}

static inline int bs_cat_escape( bytestring_sink_t* bs,const char* p,size_t n ){
